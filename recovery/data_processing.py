...
class BloodMNISTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # If RGB image, convert from HWC to CHW
...
def construct_dataloaders(dataset, defs, data_path='~/data', shuffle=True, normalize=True, exclude_num=0):
    ...
    if dataset == 'cifar10':
        ...
        loss_fn = Classification()
    elif dataset == 'bloodmnist.npz':
        trainset, validset, excluded_data, data_mean, data_std = _build_bloodmnist(path, defs.augmentations, normalize,
                                                                              exclude_num)
        loss_fn = Classification()
...
def _build_bloodmnist(data_path, augmentations=True, normalize=True, exclude_num=0):
    """Define BloodMNIST with everything considered."""
    # Load data
    ...
    def get_meanstd(images):
        """Compute the mean and std of a dataset"""
        images = images / 255.0  # Normalize pixel values to [0, 1]
        mean = np.mean(images, axis=(0, 1, 2))  # Mean over height, width, channels
        std = np.std(images, axis=(0, 1, 2))  # Std over height, width, channels
        return mean, std
    data_mean, data_std = get_meanstd(train_images)

    # Create train and validation datasets
    data, target = [], []
    for x, y in zip(train_images, train_labels):
        # Ensure the image is converted to a torch tensor
    ...
    # Optionally exclude some data from the training set
    trainset = BloodMNISTDataset(data[exclude_num:], target[exclude_num:])
    excluded_data = (data[:exclude_num], target[:exclude_num])
    ...
    valset = BloodMNISTDataset(val_images, val_labels, transform=transform)
    testset = BloodMNISTDataset(test_images, test_labels, transform=transform)

    return trainset, valset,  excluded_data, data_mean, data_std
